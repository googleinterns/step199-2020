...
function init() {
  ...
  for(var increment = 0; increment < 15; increment++) {
    points.push(new THREE.Vector3 (Math.pow(-1,increment), .20, -increment)); // GPS points
    ...
    scene.add(pivot)
    pivot.add(cylinder);
    var pivotSphereGeo = new THREE.SphereGeometry(.03); // Small sphere indicating pivot point
  ...
  gui.add(params, 'Yaw',-360, 360).step(.1).name('Yaw (degrees)').onChange(function(value) {
  ...
  params = {
	...
	gui.add(params, 'Pitch',-360, 360).step(.1).name('Pitch (degrees)').onChange(function(value) {
    for(var i = 0; i < rotationData.size; i++){
  ...
  gui.add(params, 'Roll', -360, 360).step(.1).name('Roll (degrees)').onChange(function(value) {
    for(var i = 0; i < rotationData.size; i++){
